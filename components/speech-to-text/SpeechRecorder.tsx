...
export function SpeechRecorder() {
    ...
    return (
        <Card className="w-full max-w-4xl mx-auto overflow-hidden border-0 shadow-2xl bg-card/95 backdrop-blur-xl ring-1 ring-border rounded-3xl transition-all duration-300">
            <CardContent className="p-0 min-h-[400px] flex flex-col relative h-[500px]">
                {/* Header / Top Bar */}
                <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-10 bg-gradient-to-b from-background/90 to-transparent pointer-events-none">
                    <h2 className="text-xl font-semibold text-foreground flex items-center gap-2 pointer-events-auto">
                        ...
                        Live Transcription
                        <span className="text-xs font-normal px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground ml-2">Soniox AI</span>
                    </h2>
                ...
                <div className="flex-1 p-8 pt-24 overflow-y-auto font-medium text-lg leading-relaxed text-foreground">
                    ...
                    {(!transcript && !interimTranscript) ? (
                        ...
                        <div className="space-y-1">
                            ...
                            <span className="text-primary whitespace-pre-wrap">{interimTranscript ? (transcript ? ' ' + interimTranscript : interimTranscript) : ''}</span>
                            <div ref={bottomRef} />
                        </div>
                    )}
                </div>
                ...
                <div className="p-6 bg-muted/30 border-t border-border flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <SelectTrigger className="w-[200px] bg-background border-input rounded-xl shadow-sm hover:border-primary transition-colors focus:ring-primary disabled:opacity-50">
                                <SelectValue placeholder="Language" />
                            </SelectTrigger>
                            <SelectContent>
                                {LANGUAGES.map(lang => (
                                    <SelectItem key={lang.code} value={lang.code} className="cursor-pointer">{lang.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
